...
var ffi = _interopDefault(require('ffi-napi'));
var fs = _interopDefault(require('fs-extra'));
var path = _interopDefault(require('path'));
var ref = _interopDefault(require('ref-napi'));
...
class Client {
  ...
  async fetch(query) {
    const id = crc.crc32(Math.random().toString()).toString();
    query['@extra'] = id;
...
async function getInput({ string, extras: { hint } = {} }) {
  let input = '';
  while (!input.length) {
    const type = (
...
class ClientNode extends Client {
  constructor(options = {}) {
    super('node', {
